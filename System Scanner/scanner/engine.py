...
class DiscoveryEngine:
    ...
    def _execute_module(self, module: Any) -> tuple[list[Finding], ModuleInfo]:
        ...
        try:
            ...
            logger.error("  [FAIL] %s error: %s", module_name, e, exc_info=True)
            info.status = "error"
    ...
    def run_all(self) -> tuple[list[Finding], list[ModuleInfo]]:
        ...
        """
        logger.info("Discovery Engine: Starting parallel execution of %d modules...", len(self._modules))

        all_findings: list[Finding] = []
        module_infos: list[ModuleInfo] = []

        if not self._modules:
            logger.info("Discovery Engine: No modules registered.")
        ...
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="ScannerModule") as executor:
            ...
            for future in as_completed(future_to_module):
                try:
                    ...
                    logger.error("Discovery Engine: Thread error executing %s: %s", name, e)
